          ...
          <b-card>
            ...
            <h5 slot="header">
              ...
              <span class="text-primary">
                ...
                <small
                  class="font-italic"
                >List of all registered adjustment.</small>
              </span>
            </h5>
            ...
            <b-row class="mb-2">
              ...
              <b-col sm="4">
                <b-button
                  variant="primary"
                  @click="showEntry=true, entryMode='Add', clearFields('adjustmentlist'), tables.adjustmentitems.items = [], forms.adjustmentlist.fields.adjustment_datetime = new Date()"               
                >
              ...
              <b-col sm="4">
                <b-form-input
                  v-model="filters.adjustmentlists.criteria"
                  type="text"
                  placeholder="Search"
                ></b-form-input>
              </b-col>
            ...
            <b-row>
              ...
              <b-col sm="12">
                <b-table
                  ...
                  :current-page="paginations.adjustmentlists.currentPage"
                  :per-page="paginations.adjustmentlists.perPage"
                >
                  <!-- table -->

                  <template slot="action" slot-scope="data">
                    <!-- action slot  :to="{path: 'adjustmentlists/' + data.item.id } -->
                    ...
                    <b-btn
                      :size="'sm'"
                      :disabled="forms.adjustmentlist.isDeleting"
                      variant="danger"
                      @click="setDelete(data)"
                    >
                      <icon v-if="forms.adjustmentlist.isDeleting" name="sync" spin></icon>
            ...
            <b-row>
              ...
              <b-col sm="12" class="my-1">
                <b-pagination
                  size="sm"
                  align="right"
                  :total-rows="paginations.adjustmentlists.totalRows"
                  :per-page="paginations.adjustmentlists.perPage"
                  v-model="paginations.adjustmentlists.currentPage"
                  class="my-0"
                />
              </b-col>
            </b-row>
        ...
        <b-col sm="12">
          <b-card>
            ...
            <b-form autocomplete="off">
                ...
                <b-col lg="2">
                    ...
                    <b-form-input
                        readonly
                        id="adjustment_no"
                        v-model="forms.adjustmentlist.fields.adjustment_no"
                        type="text"
                ...
                <b-col lg="3">
                    ...
                    <select2
                        ...
                        v-model="forms.adjustmentlist.fields.department_id"
                    >
                        <option
                ...
                <b-col lg="3">
                    ...
                    <date-picker
                        ...
                        format="MMMM DD, YYYY"
                        v-model="forms.adjustmentlist.fields.adjustment_datetime"
                        lang="en"
                ...
                <b-col lg="4">
                    <b-form-group>
                       ...
                       <b-form-radio-group v-model="forms.adjustmentlist.fields.adjustment_type" name="radio-sub-component" :checked="adjustment_type = 0">
            ...
            <div>
              <!-- row table -->
                <b-row class="mb-2">
                    ...
                    <b-col lg=4></b-col>
                    <b-col lg=4>
                        <b-form-input v-model="filters.adjustmentitems.criteria" placeholder="Search"></b-form-input>
                    </b-col>
                </b-row>
                <b-row>
                    <b-col sm="12">
                        <b-table
                            responsive fixed striped hover small bordered show-empty
                            :filter="filters.adjustmentitems.criteria"
                            :fields="tables.adjustmentitems.fields"
                            :items.sync="tables.adjustmentitems.items"
                        >
                    <!-- table -->
                            ...
                            <template slot="product_remarks" slot-scope="data">
                                <b-form-input v-model="data.item.product_remarks">
                        ...
                        <b-row>
                            <b-col lg=4>
                                <b-form-textarea
                                    rows=2
                                    placeholder="Remarks"
                                    v-model="forms.adjustmentlist.fields.adjustment_remarks">
                                </b-form-textarea>
                            </b-col>
                            <b-col lg=5></b-col>
                            <b-col lg=3>
            ...
            <b-row class="pull-right mt-2">
                <b-col sm="12">
                    <b-button 
                        :disabled="forms.adjustmentlist.isSaving" 
                        variant="primary" 
                        @click="onAdjustmentEntry">
                        <icon v-if="forms.adjustmentlist.isSaving" name="sync" spin></icon>
      ...
      <!-- modal div -->
        <div>
            ...
            <b-modal
                ...
                :scrollable="true"
                @shown="focusElement('adjustment_id')"
            >
...
export default {
  ...
  methods: {
    onAdjustmentEntry() {
      if(this.tables.adjustmentitems.items.length > 0) {
        this.forms.adjustmentlist.fields.items = this.tables.adjustmentitems.items
        if (this.entryMode == "Add") {
          this.createEntity("adjustmentlist", false, "adjustmentlists")
        } else {
          this.updateEntity("adjustmentlist", "adjustment_id", false, this.row)
        }
      }
      else {
        this.$notify({
    ...
    onAdjustmentDelete() {
      this.deleteEntity("adjustmentlist", this.adjustment_id, true, "adjustmentlists", "adjustment_id")
    },
    onProductlistDelete() {
      this.deleteEntity(
        "productlist",
    ...
    async setDelete(data) {
      if((await this.checkIfUsed("adjustmentlist", data.item.adjustment_id)) == true){
        this.$notify({
          type: "error",
          group: "notification",
          title: "Error!",
    ...
    setUpdate(data) {
      this.row = data.item
      this.$http.get('api/adjustmentlist/'+ data.item.adjustment_id, {
          headers: {
                Authorization: 'Bearer ' + localStorage.getItem('token')
          }
      }).then(response => {
          this.forms.adjustmentlist.fields = response.data.adjustmentlists
          this.tables.adjustmentitems.items = response.data.adjustmentitems
          this.showEntry = true
          this.entryMode = "Edit"
      }).catch(err => {
          console.log(err)
    ...
    addProduct() {
        if(this.selectedRow.length > 0){
            if(this.tables.adjustmentitems.items.filter(i => i.product_id == this.selectedRow[0].product_id).length > 0){
            this.$notify({
                    type: "error",
                    group: "notification",
                    title: "Error!",
            ...
            this.tables.adjustmentitems.items.push({
    ...
    removeProduct(index){
        this.tables.adjustmentitems.items.splice(index, 1)
    }
  },
  created() {
    ...
    this.$http.get('api/adjustmentlists', {
        headers: {
            Authorization: 'Bearer ' + localStorage.getItem('token')
        }
    }).then(response => {
        this.tables.adjustmentlists.items = response.data.adjustmentlists
        this.paginations.adjustmentlists.totalRows = response.data.adjustmentlists.length
  ...
  computed: {
    getTotalItems() {
      this.forms.adjustmentlist.fields.total_amount = 0

      this.tables.adjustmentitems.items.forEach(item => {
            this.forms.adjustmentlist.fields.total_amount += Number(item.product_cost) * Number(item.product_quantity)
        
      })
      return this.forms.adjustmentlist.fields.total_amount
    }
  }
}
</script>